...
class ConcursoController extends BaseController
{
    ...
    public function listDoFilter($filters = null)
    {
        ...
        try {
            ...
            $concursos = $concursos
                ->merge(
                    $created
                        ...
                        ->filter(function ($concurso) {
                            return $concurso->oferentes_etapa_tecnica->count() > 0;
                        })
                )
                ->merge(
                    $evaluating
            ...
            $concursos = collect();
            $concursos = $concursos->merge(
                $user->customer_company->getAllConcursosByCompany()->onlyTrashed()->get()->sortBy('id')
            );
            foreach ($concursos as $concurso) {
                array_push(
    ...
    public function delete(Request $request, Response $response, $params)
    {
        ...
        try {
            ...
            if ($success) {
                $connection->commit();
                $message = 'Concurso cancelado con 茅xito.';
    ...
    private function mapConcursoList($concurso)
    {
        return [
            ...
            'NumSolicitud' => $concurso->solicitud_compra,
        ...
        try {
            ...
            if ($params['step'] === Step::STEPS['customer']['evaluacion-reputacion']) {
                ...
                if ($concurso->oferentes->whereIn('adjudicacion', [1, 2, 3])->count() > 0) {
                    ...
                    switch ($id_adjudicacion) {
                        case 1:
                            $tipo_adjudicacion = 'Adjudicaci贸n Integral';
                            break;
                        case 2:
                            $tipo_adjudicacion = 'Adjudicaci贸n Individual';
                            break;
                        case 3:
                            $tipo_adjudicacion = 'Adjudicaci贸n Manual';
                            break;
                    }
                }
                $verOfertasEnable = false;
                ...
                $list = array_merge($list, array_merge($common_data, [
                    'titleEvaluaciones' => 'Evaluaci贸n de la ' . $concurso->tipo_concurso_nombre,
        ...
        try {
            ...
            if ($create) {
                ...
                if ($is_sobrecerrado) {
                    ...
                    $isReadOnly = ($fechasubasta < $fechaActual); // Aseg煤rate de usar ">" en lugar de ">="
                } else {
                    $isReadOnly = false; // Asume que los dem谩s casos no tienen restricciones.
                }
            }
            ...
            $breadcrumbs = [
                ...
                ['description' => $create ? 'Nuevo' : 'Edici贸n']
            ];
    ...
    private function createOrEditGo($create, $list, $user, $concurso, $is_copy)
    {
        ...
        return array_merge($list, [
            ...
            'FechaLimiteTecnica' => $create ? Carbon::now()->addDays(4)->addHour(1)->format('d-m-Y H:i') : ($concurso->fecha_limite_economicas ?
                $concurso->fecha_limite_economicas->format('d-m-Y H:i') :
                null
            ),
        ]);
    ...
    public function store(Request $request, Response $response, $params)
    {
        ...
        try {
            ...
            if ($create) {
                // Validar si el usuario fiscalizador est谩 habilitado
                if ($entity->concurso_fiscalizado == 'no') {
                    // Si no se requiere fiscalizador, aseg煤rate de que no se est茅 validando
                    $entity->UsuarioSupervisor = null; // O manejarlo seg煤n tu l贸gica
                } else {
                    // Validar si el usuario fiscalizador est谩 habilitado
                    if (!$entity->UsuarioSupervisor) {
                        return [
                            'success' => false,
                            'message' => 'Debe seleccionar un fiscalizador si se habilita la opci贸n.',
                            'status' => 422
                        ];
                    }
                }
                ...
                if ($result['error']) {
                    ...
                    $connection->commit();
                    $message = 'Concurso guardado con 茅xito.';
                    $success = true;
                }
            }
                ...
                if ($result['error']) {
                    ...
                    if (count($oferentes) > 0) {
                        ...
                        if (
                            ...
                            $template = rootPath(config('app.templates_path')) . '/email/readjustmentofdates.tpl';
                            ...
                            //Enviamos el mail a los oferentes que se envio Invitaci贸n
    ...
    public function sendSecondRound(Request $request, Response $response)
    {
        ...
        try {
            ...
            if (!$fechaNuevaCampo || !$comentarioCampo) {
                throw new Exception("Configuraci贸n inv谩lida para la ronda $nuevaRonda.");
            }
            ...
            */
            ...
            if ($validator->fails()) {
                ...
                $title = "$rondaTitle oferta econ贸micas habilitada";
                $subject = $concurso->nombre . ' - ' . $title;
                $template = rootPath(config('app.templates_path')) . '/email/enabled_second_round.tpl';
    
                foreach ($concurso->oferentes as $oferente) {
                    if ($oferente->etapa_actual === Participante::ETAPAS['economica-revisada']) {
                        $oferente->update([
                        ...
                        $html = $this->fetch($template, [
                            ...
                            'date_limit' => $fechaNuevaCampo,
                            'comentario' => $body->CommentNewRound
                        ]);
    
                        $result = $emailService->send($html, $subject, $oferente->company->users->pluck('email'), $oferente->company->business_name);
                        $success = $result['success'];
                    }
                }
                ...
                if ($success) {
                    $connection->commit();
                    $message = 'Concurso habilitado para la ' . $rondaTitle . ' con 茅xito.';
                    $redirect_url = route('concursos.cliente.serveList');
                } else {
                    $connection->rollBack();
        ...
        foreach ($oferentes as $index => $oferente) {
            ...
            for ($round = 1; $round <= $techRound; $round++) {
                ...
                if ($round == $techRound) {
                    ...
                    $evaluation->newRound = $techRound < 5 ? Participante::RONDAS[$techRound + 1] . ' T茅cnica' : Participante::RONDAS[$techRound];
    ...
    private function storeSheets($concurso, $entity)
    {
        ...
        foreach ($entity->Sheets as $sheet) {
            switch ($sheet->action) {
                case 'upload':
                    // Si hab铆a un archivo previo, lo eliminamos.
                    if ($sheet->id) {
    ...
    private function storePayroll($concurso, $body, $createOrEdit)
    {
        ...
        if ($createOrEdit === 'create') {
            if ($body->IncluyePrecalifTecnica === 'no') {
                return [
                    'success' => true,
                    'message' => 'No incluye Etapa T茅cnica',
                    'technicalAdded' => false,
                    'technicalDeleted' => false,
                    'technicalChanged' => false
                ];
            ...
            if ($body->IncluyePrecalifTecnica === 'si') {
                ...
                $atributos[] = [
                    'id' => 0,
                    'atributo' => 'Puntaje m铆nimo necesario',
                ...
                return [
                    'success' => true,
                    'message' => 'Agregada Plantilla T茅cnica',
                    'technicalAdded' => true,
                    'technicalDeleted' => false,
                    'technicalChanged' => false
                ];
        ...
        if ($createOrEdit === 'edit') {
            if ($body->IncluyePrecalifTecnica === 'no') {
                if ($concurso->plantilla_tecnica) {
                    ...
                    return [
                        'success' => true,
                        'message' => 'Se ha eliminado la precalificaci贸n t茅cnica',
                        'technicalAdded' => false,
            ...
            if ($body->IncluyePrecalifTecnica === 'si') {
                ...
                if ($concurso->plantilla_tecnica) {
                    ...
                    if ($oldPayroll[0]->puntaje != $newPayRoll[0]->puntaje) {
                        $success = true;
                        $message = 'Se ha Cambiado la precalificaci贸n t茅cnica';
    ...
    {
        // Eliminamos los que todav铆a est茅n en estado seleccionado (no se enviaron invitaciones)
        $concurso->refresh();
    ...
    private function mapAdditionalDocuments($entity)
    {
        ...
        // Documentos Adicionales Veh铆culo
            ...
            'finalizacion_consultas' => [
                ...
                function ($attribute, $value, $fail) use ($body) {
                    ...
                    if ($fecha_limite->diffInDays($finalizacion_consultas, false) < 1)
                        $fail('La fecha de Muro de Consultas debe ser al menos 1 d韆s mayor a la fecha de Aceptaci髇.');

                    if ($fecha_limite_economicas != null)
                        if ($finalizacion_consultas->diffInDays($fecha_limite_economicas, false) < 1)
                            $fail('La fecha de Muro de Consultas debe ser al menos 1 d韆 menor a la fecha de Presentaci髇 Econ髆ica.');
                }
            ]
        ];
                        ...
                        if ($body->IncluyePrecalifTecnica == 'si') {
                            if ($body->FechaLimiteTecnica) {
                                ...
                                if ($ficha_tecnica_fecha_limite->diffInDays($inicio_subasta, false) < 1)
                                    $fail('La fecha de Inicio de Concurso debe ser al menos 1 d韆 mayor a la de Presentaci髇 T閏nica.');
                            }
                        } else {
                            $fecha_limite = Carbon::createFromFormat('d-m-Y H:i', $body->FechaLimite);
                            if ($fecha_limite->diffInDays($inicio_subasta, false) < 1)
                                $fail('La fecha de Inicio de Concurso debe ser al menos 1 d韆 mayor a la fecha de Aceptaci髇.');
                        }
                        if ($finalizacion_consultas != null)
                            if ($finalizacion_consultas->diffInDays($inicio_subasta, false) < 1)
                                $fail('La fecha de Inicio de Concurso debe ser al menos 1 d韆 mayor a la fecha de Muro de Consultas.');
                    },
                ]
            ]);
        }
                ...
                'fecha_limite_economicas' => [
                    ...
                    function ($attribute, $value, $fail) use ($body) {
                        ...
                        if ($fecha_limite->diffInDays($fecha_limite_economicas, false) < 3) {
                            $fail('La fecha de Presentaci髇 Econ髆ica debe ser al menos 3 d韆s posterior a la fecha de Aceptaci髇.');
                        }
                    },
                ]
            ]);
                        ...
                        function ($attribute, $value, $fail) use ($body) {
                            if ($value) {
                                ...
                                if ($fecha_limite->diffInDays($ficha_tecnica_fecha_limite, false) < 2)
                                    $fail('La fecha de Presentaci髇 T閏nica debe ser al menos 2 d韆s mayor a la fecha de Aceptaci髇.');

                                if ($fecha_limite_economicas != null)
                                    if ($ficha_tecnica_fecha_limite->diffInDays($fecha_limite_economicas, false) < 1)
                                        $fail('La fecha de Presentaci髇 T閏nica debe ser al menos 1 d韆 menor a la fecha de Presentaci髇 Econ髆ica.');
                            }
                        }
                    ]
                ]);
                        ...
                        if ($body->IncluyePrecalifTecnica == 'si') {
                            if ($body->FechaLimiteTecnica) {
                                ...
                                if ($finalizacion_consultas->greaterThan($ficha_tecnica_fecha_limite)) {
                                    if ($finalizacion_consultas->diffInDays($fecha_limite_economicas, false) < 1)
                                        $fail('La fecha de Presentaci髇 Econ髆ica debe ser al menos 1 d韆 mayor a la fecha de Muro de Consulta.');
                                } else {
                                    if ($ficha_tecnica_fecha_limite->diffInDays($fecha_limite_economicas, false) < 1)
                                        $fail('La fecha de Presentaci髇 Econ髆ica debe ser al menos 1 d韆 mayor a la de Presentaci髇 T閏nica.');
                                }
                            }
                        } else {
                            $fecha_limite = Carbon::createFromFormat('d-m-Y H:i', $body->FechaLimite);
                            ...
                            if ($finalizacion_consultas->diffInDays($fecha_limite_economicas, false) < 1)
                                $fail('La fecha de Presentaci髇 Econ髆ica debe ser al menos 1 d韆 mayor a la fecha de Muro de Consulta.');
                            if ($fecha_limite->diffInDays($fecha_limite_economicas, false) < 3)
                                    $fail('La fecha de Presentaci髇 Econ髆ica debe ser al menos 3 d韆s mayor a la fecha para aceptar la invitaci髇.');
                        }
                    },
                ]
            ]);
    ...
    private function validatePayroll($fields)
    {
        ...
        return validator(
            ...
            $rules = array_merge($common_rules, $conditional_rules),
            $messages = [
                'total.in' => 'Los valores de ponderaci贸n deben sumar 100% (valor actual ' . $fields['total'] . '%)',
    ...
    private function validateProducts($body, $fields, $is_sobrecerrado, $is_online)
    {
        ...
        return validator(
            ...
            $rules = array_merge($common_rules, $conditional_rules),
            $messages = [
                'products.*.nombre.max' => 'El campo Nombre Item no puede ser superior a 255 car谩cteres',
            ]
...
*/
            $ajustdate =
                ($fechaInvitacionEdit || $fechaFinConsultasEdit /*|| $fechaTecnicaLimitEdit || $fechaEconomicLimitEdit*/) ? true : false;
            ...
            if ($apuChange) {
                $ajustDocumentsEconomica += $concurso->apu === 'si' ?
                    ['apu' => 'No se requiere An谩lisis de Precios Unitarios'] : ['apu' => 'Se requiere An谩lisis de Precios Unitarios'];
            }
    ...
    {
        $documents = [
            'seguro_caucion' => 'P贸liza de seguro de cauci贸n',
            ...
            'obras_memoria' => 'Memoria t茅cnica',
            'obras_antecedentes' => 'Antecedentes de obras similares',
            'tarima_ficha_tecnica' => 'Ficha T茅cnica de la tarima',
            'tarima_licencia' => 'Licencia Ambiental integral (LAI)',
            'tarima_nom_144' => 'Cumplimiento NOM-144 SEMARNAT 2017',
            'tarima_acreditacion' => 'Acreditaci贸n legal con la procedencia de la madera',
            'edificio_balance' => '脷ltimo balance de la empresa',
            ...
            'edificio_gestion' => 'Gesti贸n de H&S (incluir indicadores, procedimientos, detalle de personal, etc.)',
            'edificio_maquinas' => 'Listado de m谩quinas y equipos a utilizar',
        ];
