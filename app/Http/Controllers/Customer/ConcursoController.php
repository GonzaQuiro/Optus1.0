...
class ConcursoController extends BaseController
{
    ...
    {   
        ...
        if ($expectedToken !== $sessionToken) {
            return $this->json($response, [
                'success' => false,
                'message' => 'Acceso no autorizado. Token inv谩lido para acceder al detalle del concurso.'
            ], 403);
        }

        // 馃敟 Invalida el token despu茅s de usarlo
    ...
    {   
        ...
        if ($expectedToken !== $sessionToken) {
            return $this->json($response, [
                'success' => false,
                'message' => 'Acceso no autorizado. Token inv谩lido para editar el concurso.'
            ], 403);
        }
    ...
    public function serveCreate(Request $request, Response $response, $params)
    {
        ...
        if ($concurso_id) {
            ...
            if (!$storedToken || $storedToken !== $expectedToken) {
                return $this->json($response, [
                    'success' => false,
                    'message' => 'Acceso no autorizado. Token inv谩lido para copiar concurso.'
                ], 403);
            }
        ...
        try {
            ...
            */
            ...
            if ($validator->fails()) {
                ...
                // 2) Para la 2陋 ronda, comprobamos directamente en BD si quedan oferentes en 'economica-revisada'
                if ($nuevaRonda === 2) {
                ...
                // 3) Env铆o mails solo a los pendientes
                $success = true;
            ...
            'finalizacion_consultas' => [
                ...
                function ($attribute, $value, $fail) use ($body) {
                    ...
                    if ($fecha_limite->diffInDays($finalizacion_consultas, false) < 1)
                        $fail('La fecha de Muro de Consultas debe ser al menos 1 d铆as mayor a la fecha de Aceptaci贸n.');
/*
                    if ($fecha_limite_economicas != null)
                        if ($finalizacion_consultas->diffInDays($fecha_limite_economicas, false) < 1)
                            $fail('La fecha de Muro de Consultas debe ser al menos 1 d铆a menor a la fecha de Presentaci贸n Econ贸mica.');
*/
                }
            ]
        ];
                        ...
                        if ($body->IncluyePrecalifTecnica == 'si') {
                            if ($body->FechaLimiteTecnica) {
                                ...
                                if ($ficha_tecnica_fecha_limite->diffInDays($inicio_subasta, false) < 1)
                                    $fail('La fecha de Inicio de Concurso debe ser al menos 1 d铆a mayor a la de Presentaci贸n T茅cnica.');
                            }
                        } else {
                            $fecha_limite = Carbon::createFromFormat('d-m-Y H:i', $body->FechaLimite);
                            if ($fecha_limite->diffInDays($inicio_subasta, false) < 1)
                                $fail('La fecha de Inicio de Concurso debe ser al menos 1 d铆a mayor a la fecha de Aceptaci贸n.');
                        }
                        if ($finalizacion_consultas != null)
                            if ($finalizacion_consultas->diffInDays($inicio_subasta, false) < 1)
                                $fail('La fecha de Inicio de Concurso debe ser al menos 1 d铆a mayor a la fecha de Muro de Consultas.');
                    },
                ]
            ]);
        }
                ...
                'fecha_limite_economicas' => [
                    ...
                    function ($attribute, $value, $fail) use ($body) {
                        ...
                        if ($fecha_limite->diffInDays($fecha_limite_economicas, false) < 3) {
                            $fail('La fecha de Presentaci贸n Econ贸mica debe ser al menos 3 d铆as posterior a la fecha de Aceptaci贸n.');
                        }
                    },
                ]
            ]);
                        ...
                        function ($attribute, $value, $fail) use ($body) {
                            if ($value) {
                                ...
                                if ($fecha_limite->diffInDays($ficha_tecnica_fecha_limite, false) < 2)
                                    $fail('La fecha de Presentaci贸n T茅cnica debe ser al menos 2 d铆as mayor a la fecha de Aceptaci贸n.');

                                if ($fecha_limite_economicas != null)
                                    if ($ficha_tecnica_fecha_limite->diffInDays($fecha_limite_economicas, false) < 1)
                                        $fail('La fecha de Presentaci贸n T茅cnica debe ser al menos 1 d铆a menor a la fecha de Presentaci贸n Econ贸mica.');
                            }
                        }
                    ]
                ]);
                        ...
                        if ($body->IncluyePrecalifTecnica == 'si') {
                            if ($body->FechaLimiteTecnica) {
                                ...
                                if ($finalizacion_consultas->greaterThan($ficha_tecnica_fecha_limite)) {
                                    if ($finalizacion_consultas->diffInDays($fecha_limite_economicas, false) < 1)
                                        $fail('La fecha de Presentaci贸n Econ贸mica debe ser al menos 1 d铆a mayor a la fecha de Muro de Consulta.');
                                } else {
                                    if ($ficha_tecnica_fecha_limite->diffInDays($fecha_limite_economicas, false) < 1)
                                        $fail('La fecha de Presentaci贸n Econ贸mica debe ser al menos 1 d铆a mayor a la de Presentaci贸n T茅cnica.');
                                }
                            }
                        } else {
                            $fecha_limite = Carbon::createFromFormat('d-m-Y H:i', $body->FechaLimite);
                            ...
                            if ($finalizacion_consultas->diffInDays($fecha_limite_economicas, false) < 1)
                                $fail('La fecha de Presentaci贸n Econ贸mica debe ser al menos 1 d铆a mayor a la fecha de Muro de Consulta.');
                            if ($fecha_limite->diffInDays($fecha_limite_economicas, false) < 3)
                                    $fail('La fecha de Presentaci贸n Econ贸mica debe ser al menos 3 d铆as mayor a la fecha para aceptar la invitaci贸n.');
                        }
                    },
                ]
            ]);
