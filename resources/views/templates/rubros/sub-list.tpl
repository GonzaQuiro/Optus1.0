    ...
    <div class="col-md-12">
        <div class="portlet light bordered">
            <div class="portlet-title">
                <div class="btn-group">
                    <a 
                        data-bind="attr: {literal}{ href: '/configuraciones/rubros/' + CategoryId() + '/subrubros/nuevo' }{/literal}" 
            ...
            <div class="portlet-body">
                ...
                <table class="table table-striped table-bordered" id="listaSubrubros">
                    <thead>
                        <tr>
                            <th>Nombre</th>
                    ...
                    <tbody data-bind="dataTablesForEach : { data: ListaSubrubros, options: { paging: true }}">
                        <tr>
                            <td class="col-md-6" data-bind="text: Nombre"></td>
                            <td class="col-md-4 text-center">
                                <a data-bind="attr: {literal}{ href: '/configuraciones/rubros/' + $root.CategoryId() + '/subrubros/edicion/' + Id }{/literal}" class="btn btn-xs green" title="Editar">
                                    Editar 
...
var ConfiguracionesSubRubrosListado = function (data) {
    var self = this;

    this.Breadcrumbs = ko.observableArray(data.breadcrumbs);
    this.CategoryId = ko.observable(data.list.CategoryId);
    this.CategoryName = ko.observable(data.list.CategoryName);
    this.ListaSubrubros = ko.observableArray(data.list.SubRubros);

    this.Eliminar = function (id) {
        swal({
            title: '¿Está seguro de que desea eliminar el sub-rubro?',
            type: 'info',
            ...
            if (result) {
                ...
                var url = '/configuraciones/rubros/' + params[2] + '/subrubros/delete/' + id;
...
jQuery(document).ready(function () {
    ...
    var url = '/configuraciones/rubros/' + params[2] + '/subrubros/list';
    Services.Get(url, data, 
        (response) => {
            if (response.success) {
                window.E = new ConfiguracionesSubRubrosListado(response.data);
                AppOptus.Bind(E);
            }
            $.unblockUI();
        }, 
