...
var ConfiguracionesUnidades = function (data) {
    var self = this;
    ...
    this.Save = function () {
        $.blockUI();
        var url = '/configuraciones/unidades/save';
        switch (params[2]) {
            case 'edicion':
                url += '/' + params[3];
                break;
...
jQuery(document).ready(function () {
    ...
    var url = '/configuraciones/unidades/';
    switch (params[2]) {
        case 'nuevo':
            url += 'nuevo/data';
            break;
    ...
    Services.Get(url, data, 
        (response) => {
            if (response.success) {
                window.E = new ConfiguracionesUnidades(response.data);
                AppOptus.Bind(E);
            }
            $.unblockUI();
        }, 
