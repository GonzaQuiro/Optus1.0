...
<div class="row">
    <div class="col-md-12 text-right">
        <a 
            href="/configuraciones/catalogos" 
            type="button" 
            class="btn default">
            Volver a Listado
        </a>
...
var ConfiguracionesCatalogos = function (data) {
    var self = this;
    ...
    this.Save = function () {
        $.blockUI();
        var url = '/configuraciones/catalogos/save';
        switch (params[2]) {
            case 'edicion':
                url += '/' + params[3];
                break;
...
jQuery(document).ready(function () {
    ...
    var url = '/configuraciones/catalogos/';
    switch (params[2]) {
        case 'nuevo':
            url += 'nuevo/data';
            break;
    ...
    Services.Get(url, data, 
        (response) => {
            if (response.success) {
                window.E = new ConfiguracionesCatalogos(response.data);
                AppOptus.Bind(E);
            }
            $.unblockUI();
        }, 
